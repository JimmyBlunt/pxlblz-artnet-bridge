...
// canvas â€” no margin, which previously made 3D layouts ~40px shorter than 2D. The
...
export function Preview({
  ...
  useEffect(() => {
    ...
    // Opening a map (editor map mode) must NOT touch the preview â€” it changes the
    ...
    // changes no preview input (previewSource/activeMapId/â€¦), so this loop isn't
    ...
    // known before resolving its layout â€” it drives Recommended/default ordering,
    // never a map filter.
    ...
    // The active map/shape/surface/count already carry any demo recommendation â€”
    ...
    try {
      ...
      //   sliders/toggles: convention `sliderFoo`/`toggleFoo` â†” `foo` (lowercase
    ...
    // 3D layout: hand the orbit renderer the cube's [0,1]Â³ positions, a square
    ...
    if (positions3D) {
      ...
      // Every 2D layout â€” stock plane, ring/cloud, or a 1D shape embedding â€” draws
  ...
  // one pass that composes per-pattern override â†’ recommended (demos) â†’ global-sticky
  // â†’ dev-default for every field and pushes the result into mapStore.active* +
  ...
  // The renderer then reads the seeded live values per frame as before â€” the resolver
  ...
  // does not own `sample`, so wrap density only changes where dots are drawn â€” a
  ...
  // 3D layout is active and auto-orbit is armed â€” decoupled from the pattern's
  ...
  // preview isn't painting, so â€” like every other deck control below â€” force one
  ...
  // like a physical diffuser â€” no whole-frame blur, so cores stay crisp, the array
  ...
  // which folds a normalÂ·viewDir multiplier into the 3D per-vertex brightness when
  ...
  // store â€” firing the control effects above, which repaint a paused preview â€” and
  ...
  useEffect(() => {
    ...
    w.__pxlblz = {
      ...
      capture: (name = 'capture.png') => {
        ...
        // effects (diffusion/solidity push into the renderer) flush first â€”
      ...
      // Swap the previewed source without touching pattern records â€” lets the
      ...
      captureSequence: async (opts: CaptureSequenceOptions) => {
        ...
        // (#879) so every run â€” and every --start â€” sees the same camera.
    ...
    >
      ...
      <div ref={containerRef} className={constrainCanvasHeight ? 'relative w-full min-h-[100px] flex-1 overflow-hidden flex items-center justify-center' : 'relative w-full shrink-0'}>
        <div className="relative block w-fit">
          ...
          {/* Orbit viewport controls â€” gated on the active layout's display
